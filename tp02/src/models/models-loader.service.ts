...
/**
 * Extras A & B: where the initial catalogue comes from.
 *
 * Kept apart from `ModelsService` on purpose: the service decides what a
 * model is and how the catalogue is queried, this class only knows how to
 * fetch one from the outside world (a file, an HTTP API) and turn it into
 * `Model[]`. Neither needs to know about the other's concerns.
 */
@Injectable()
export class ModelsLoaderService {
  private readonly logger = new Logger(ModelsLoaderService.name);

  /**
   * The API is the source of truth, the local file is the fallback when it
   * is unreachable.
   */
   ...
   * It is wrapped in a `Promise` so `load()` can await it — left alone, a
   * callback finishes whenever it wants, possibly after the app booted.
   */
